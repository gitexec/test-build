...
from load_lake_to_pii_users import Runner, RuntimeOptions
...
def run(env, run_pipeline, sql_templates_path, bigquery):
    ...
    sql = parse_template(
        f'{sql_templates_path}/lake_to_pii.users.sql', **checkpoint)
    ...
    rs = bigquery.query(f'select * from pii.users WHERE ingestion_timestamp >= "{start}"')
    return rs
...
@skipif_prd
def test_world_ventures_payload(run):
    payloads = list(filter(lambda x: x['icentris_client'] == 'worldventures', run))
    assert len(payloads) == 1


@skipif_prd
def test_bluesun_payload(run):
    payloads = list(filter(lambda x: x['icentris_client'] == 'bluesun', run))
    assert len(payloads) == 1
